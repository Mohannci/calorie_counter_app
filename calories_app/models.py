...
class Profile(models.Model):
	...
	def save(self, *args, **kwargs):# new
		if self.food_selected != None:
			...
			calories_app = Profile.objects.filter(person_of=self.person_of).last()
			PostFood.objects.create(profile=calories_app,food=self.food_selected,calorie_amount=self.calorie_count,amount=self.quantity)
